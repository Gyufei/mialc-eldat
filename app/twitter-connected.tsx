...
export default function TwitterConnected({ size = 'lg' }: { size?: 'lg' | 'sm' }) {
  const { user, linkTwitter } = usePrivy();
  const twitterName = user?.twitter?.username;
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden">
      ...
      >
        ...
        >
          <span>
            <Twitter fillColor={twitterName ? '#B8B4FE' : '#52525B'} />
          </span>
        </span>
        <div
          className={cn(
          ...
          <div className={cn('flex flex-col flex-1 min-w-0', size === 'sm' ? 'gap-1' : 'gap-2')}>
            ...
            >
              ...
              >
                ...
                >
                  {twitterName ? `@${twitterName}` : 'Twitter / X'}
                </span>
              </div>
            </h4>
            {!twitterName && (
