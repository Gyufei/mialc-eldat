...
import Farcaster from './icon/farcaster';

export default function FarcasterConnected({ size = 'lg' }: { size?: 'lg' | 'sm' }) {
  const { user, linkFarcaster } = usePrivy();
  const farcasterName = user?.farcaster?.username;

  function handleLinkFarcaster() {
    if (farcasterName) {
      return;
    }
    linkFarcaster();
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden">
      ...
      >
        ...
        >
          <span>
            <Farcaster fillColor={farcasterName ? '#B8B4FE' : '#52525B'} />
          </span>
        </span>
        <div
          className={cn(
