...
import Tg from './icon/tg';

export default function TelegramConnected({ size = 'lg' }: { size?: 'lg' | 'sm' }) {
  const { user, linkTelegram } = usePrivy();
  const telegramName = user?.telegram?.username;
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden">
      ...
      >
        ...
        >
          <span>
            <Tg fillColor={telegramName ? '#B8B4FE' : '#52525B'} />
          </span>
        </span>
        <div
          className={cn(
          ...
          <div className={cn('flex flex-col flex-1 min-w-0', size === 'sm' ? 'gap-1' : 'gap-2')}>
            ...
            >
              ...
              >
                ...
                >
                  {telegramName ? `@${telegramName}` : 'Telegram'}
                </span>
              </div>
            </h4>
            {!telegramName && (
